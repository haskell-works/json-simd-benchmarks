...
extern __m128i simd_phi_table_32       [256];
...
void sm_process_chunk(
  ...
  for (size_t i = 0; i < in_length; i += 1) {
    __m128i p = _mm_shuffle_epi8(simd_phi_table_32[in_buffer[i]], s);
    out_phi_buffer[i] = _mm_extract_epi32(p, 0);
    s = _mm_shuffle_epi8(simd_transition_table_32[in_buffer[i]], s);
  }
